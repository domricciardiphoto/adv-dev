var pg1 = 0,
    ...
    literalhash = '',
    hashconvert = window.location.hash.replace('#' ,'');	

   function nohashbroken() {
	    "use strict";
	   if($('#pageof').val() === 'NaN') {$('#menuFSI0').click();} }	
	   
		if (hashconvert === 'Video_Games') {
		hashconvert = 'Video Games';
		}
	    if (hashconvert === 'Home_Automation') {
		hashconvert = 'Home Automation';
		}
	    if (hashconvert === 'Outdoor_Living') {
		hashconvert = 'Outdoor Living';
		}
	    if (hashconvert === 'Activity_Trackers') {
		hashconvert = 'Activity Trackers';
		}
	    if (hashconvert === 'Outdoor_Living') {
		hashconvert = 'Outdoor Living';
		}
	    if (hashconvert === 'Mattresses_&amp;_Recliners') {
		hashconvert = 'Mattresses &amp; Recliners';
		}
	    if (hashconvert === 'Drones_&amp;_Cameras') {
		hashconvert = 'Drones &amp; Cameras';
		}
	  if(hashconvert === 'Appliances' || 'TVs' || 'Recliners' || 'BBQs' || 'Mattresses' || 'Vacuums' || 'Computers' || 'Audio' || 'Drones &amp; Cameras' || 'Video Games' || 'ACs' || 'Activity Trackers' || 'Outdoor Living' || 'Mattresses &amp; Recliners'  ) {
		literalhash1 = '#'+$(".fsi[data2='"+hashconvert+"']:first").attr('id');      //'#'+$(".fsi[data2='"+hashconvert+"']:first").attr('id').replace('pg' ,'');  
		literalhash = literalhash1.replace('pg' , '');
		if( $('#'+$(".fsi[data2='"+hashconvert+"']:first").attr('id')).length )  
       { window.location.hash = literalhash;}
		setTimeout(nohashbroken , 300);
	  }
function marketing() {
 ...
 if ($('#fsi_wrapper').width() < 700	 ){
	...
	"#0" === location.hash ? ($("#pg0").load($("#pg0").attr("data"), $(".left_arrow").css('display', 'none'), function () {
		...
		return !$(this).attr("myclicker") && ($(".menu_item").removeAttr("myclicker").removeClass("menu_clicked_class"),$('.left_arrow').fadeIn(),$(".promo_motion_data").hide(), location.hash = $(this).attr("data"),$('#menu_box_desktop_name h2').text($("#pg" + location.hash.replace("#", "")).attr('data2')), catnamesys(),
			...
			left: "0"
		}), $(".menu_move_top_right").show(), $(".menu_move_top_left").hide()), 15 > pages_amount && $(".menu_move_top_right").hide(),marketing(), void $(this).addClass("imclicked").addClass("menu_clicked_class").attr("myclicker", "0"))
