...
$('#submenu').bind('touchstart', function(){
 ...
 if ($('#fsi_wrapper').width() < 700	 ){
	...
	if ($('#fsi_wrapper').width() > 751	 ){
     $('.submenu_overflow').css('width' , menu_length * 139 + 'px');
...
function runajaxcontent3() {
	...
	myjax6 >= myjax5 ? $.ajax({
		...
		cache: false,
		success: function (a) {
			$(myjax7).children(".promo_des_price").html($(".price-row:eq(0)", a).html() + " " + $(".price-row:eq(1)", a).html()), $(myjax7).children(".pdp-price-note").not(".ajax_filled").html($(".p-text p", a).html()).addClass("ajax_filled"), $("div.promo_save_note.ajax_filled p.title:contains('See Details in Checkout')").addClass("seedetailsincheckout"), $(myjax7).children('.promo_des_price:contains("undefined undefined")').text("Unavailable"), $(myjax7).not(".singlepromo-multi").wrap("<a title='Click for " + $(myjax7).attr("data") + "' href='http://www.pcrichard.com/catalog/catalog-product.jsp?prodId=" + $(myjax7).attr("data") + "'></a>"), $(myjax7 + ".singlepromo-multi").wrap("<a title='Click for " + $(myjax7).attr("data2") + "' href='" + $(myjax7).attr("data2") + "'></a>"), myjax5++, runajaxcontent()
		},
		error: function (a) {
			runajaxcontent(), myjax5++
		}
...
function wrapprogram() {
		...
		$(this).not('.finished').attr('width' , '100%').removeAttr('height').removeAttr('id').addClass('finished');
		}),$("table img").not('.finished').each(function () {	
		...
		if(tablerez.indexOf("/") > -1) {	
		$(this).attr("data") && $(this).wrap("<a title='Click for " + $(this).attr("data") + "' href='" + $(this).attr("data") + "'></a>").addClass('finished');
		}	
		else {
		$(this).attr("data") && $(this).wrap("<a title='Click for " + $(this).attr("data") + "' href='http://www.pcrichard.com/catalog/catalog-product.jsp?prodId="+ $(this).attr("data") + "'></a>").addClass('finished');	
		}
		}			
	})
}
...
$(document).ready(function () {
	"use strict";
	for ($(".promo_box").each(function () {
			$(this).attr("id", "p" + nu), $(this).append('<div class="dynamic"></div>'), nu++
		}), $("table td img").each(function () {
			$(this).removeAttr("height")
		}), $("#pages_total").text(pages_amount), -1 !== location.href.indexOf("#") ? ($("#pg" + location.hash.replace("#", "")).show(), $("#pageof").text(propernumber)) : ($("#pg0").delay(600).show(), location.hash = "0", $("#pageof").text("1")), 1 === show_additional_menu && $("#submenu").show(), $(".fsi").each(function () {
	...
	"#0" === location.hash ? ($("#pg0").load($("#pg0").attr("data"), $(".left_arrow").css('display', 'none'), function () {
		...
		return !$(this).attr("myclicker") && ($(".menu_item").removeAttr("myclicker").removeClass("menu_clicked_class"),$('.left_arrow').fadeIn(),$(".promo_motion_data").hide(), location.hash = $(this).attr("data"),$('#menu_box_desktop_name h2').text($("#pg" + location.hash.replace("#", "")).attr('data2')), catnamesys(),
											  ...
											  $("#pageof").text(eval(parseInt($(this).attr("data")) + 1)), $(this).addClass("imclicked").addClass("menu_clicked_class"), "slide" === animation_type ? ($(".fsi").hide(), $("#pg" + location.hash.replace("#", "")).slideDown()) : ($(".fsi").hide(), $("#pg" + location.hash.replace("#", "")).show()), $(this).attr("data") > page_change_movement ? 
		...
		swipeLeft: function () {
			swipeinfo = parseInt(location.hash.replace("#", "")) + 1, $("#menuFSI" + eval(swipeinfo)).click(), $("html, body").animate({
				scrollTop: "0px"
			})
		},
		swipeRight: function () {
