var $menu_length = $('.submenu_button').length,
	...
	loadpage = "",
	opencat = "",
	$submenuOverflow = $('.submenu_overflow'),
	$fsiWrapperWidth = $('#fsi_wrapper').width(),
	runajaxcontent2,
	runajaxcontent3,
	$rac2Url = $(myjax7).attr("data"),
	$rac3Url = "/global/modals/modal-pdp-qv.jsp?modal=true&prodId=" + $(myjax7).attr("data"),
	$subMenu = $("#submenu");
...
function runAjax(ajaxUrl, successCB, errCB, arg1, arg2){
	"use strict";
	if(arg1 >= arg2){
		$.ajax({ url: ajaxUrl, timeout: 4e3, type: "GET", cache: false })
		.done( successCB )
		.fail( errCB );
	} else {
...
runajaxcontent2 = runAjax( $rac2Url, rac2, handleError, myjax6, myjax5 );

runajaxcontent3 = runAjax( $rac3Url, rac3, handleError, myjax6, myjax5 );

function wrapProgramFx(selector){
	$(selector).not('.finished').each(function () {
...
function submenuOverflowFx(size) {
	return $submenuOverflow.css('width' , $menu_length * size + 'px');
}

	$( window ).resize(function() {

	 switch(true) {
		case $fsiWrapperWidth === 751:
			submenuOverflowFx(110);
			$submenuOverflow.css('width' , $menu_length * 110+ 'px');
			break;
		case $fsiWrapperWidth === 750:
			submenuOverflowFx(110);
			$submenuOverflow.css('width' , $menu_length * 110+ 'px');
			break;
		case $fsiWrapperWidth < 700:
			submenuOverflowFx(95);
			$submenuOverflow.css('width' , $menu_length * 95 + 'px');
			break;
		case $fsiWrapperWidth > 751:
			submenuOverflowFx(110);
			$submenuOverflow.css('width' , $menu_length * 110 + 'px');
