...
$(document).ready(function () {
	...
	for ($(".promo_box").each(function () {
			...
			$(this).removeAttr("height")
		}), $("#pages_total").text(pages_amount), -1 !== location.href.indexOf("#") ? ($("#pg" + location.hash.replace("#", "")).show(), $("#pageof").text('Page '+propernumber)) : ($("#pg0").delay(600).show(), location.hash = "0", $("#pageof").text("Page 1")), 1 === show_additional_menu && $("#submenu").show(), $(".fsi").each(function () {
	...
	"#0" === location.hash ? ($("#pg0").load($("#pg0").attr("data"), $(".left_arrow").css('display', 'none'), function () {
		...
		return !$(this).attr("myclicker") && ($(".menu_item").removeAttr("myclicker").removeClass("menu_clicked_class"),$('.left_arrow').fadeIn(),$(".promo_motion_data").hide(), location.hash = $(this).attr("data"),$('#menu_box_desktop_name h2').text($("#pg" + location.hash.replace("#", "")).attr('data2')), catnamesys(),
											  ...
											  $("#pageof").text('Page '+eval(parseInt($(this).attr("data")) + 1)), $(this).addClass("imclicked").addClass("menu_clicked_class"), "slide" === animation_type ? ($(".fsi").hide(), $("#pg" + location.hash.replace("#", "")).slideDown()) : ($(".fsi").hide(), $("#pg" + location.hash.replace("#", "")).show()), $(this).attr("data") > page_change_movement ? 
